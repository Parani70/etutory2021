          ...
          <div class="row">
            <div class="col-md-6">
              <div class="card">
                ...
                <form role="form" action="{{url('/saveexampayment') }}" method="post" enctype="multipart/form-data">
                  {{csrf_field()}}
                  ...
                  <div class="card-body">
                    ...
                    @if($dataset['fromshoppingCart'] == 'YES')
                    ...
                    <table id="questions-datatable" class="table">
                      ...
                      <tbody>
                        ...
                        @foreach($cartdatasub as $cartdata)
                    ...
                    @if($dataset['discountedPrice'] > 0)
                    <p>Amount : <b>{{$dataset['price']}} LKR</b></p>
                    <p>Discounted Amount : <b>{{$dataset['discountAmount']}} LKR</b></p>
                    <p>Amount to be Paid : <b>{{$dataset['discountedPrice']}} LKR</b></p>
                    @else
                    ...
                    @if($dataset['discountType'] == 'buyx')
                    <p><span class=" badge bg-success">You will get follwing product FREE for the promo code "{{$dataset['promocode']}}"</span></p>
                    <ul>
                      @php
                      $ar = 1;
                      @endphp
                      @foreach($dataset['getFreeArray'] as $freeProductEntry)

                      @php
                      $paperTemplateDetails = DB::table('papertemplates')->where([
                      ['id', '=', $freeProductEntry],

                      ])->get();
                      @endphp
                      <li>{{$paperTemplateDetails[0]->coursename}}</li>
                      <input type="hidden" value="{{$freeProductEntry}}" name="freeArray_id{{ $ar}}">
                      <input type="hidden" value="{{$paperTemplateDetails[0]->coursename}}" name="freeArray_ename{{ $ar}}">
                      @php
                      $ar ++;
                      @endphp
                    ...
                    <div class="row" id="uploadbox" style="display: none;">
                      ...
                      <div class="col-sm-6">
                        <div class="custom-file">
                          <input type="file" name="depositimage" class="custom-file-input" style="height:100%;" id="customFile">
                      ...
                      <input type="hidden" id="examprice" value="{{$dataset['price']}}" name="examprice">
                      <input type="hidden" id="producttype" value="{{$dataset['type']}}" name="producttype">
                      ...
                      <input type="hidden" id="promocode" value="{{$dataset['promocode']}}" name="promocode">
                      @php
                      $arrayCount = count($dataset['getFreeArray']);
                      @endphp


                      <input type="hidden" id="getFreeArray" value="{{$arrayCount}}" name="getFreeArraycount">
                      <input type="hidden" id="discountType" value="{{$dataset['discountType']}}" name="discountType">
                      <input type="hidden" id="getFreeArray" value="{{$dataset['fromshoppingCart']}}" name="fromshoppingCart">
                ...
                <div class="cardform">
                  ...
                  <form role="form" action="{{url('/cardpayproceed') }}" method="post" enctype="multipart/form-data">
                    ...
                    <input type="hidden" id="getFreeArray" value="{{$dataset['discountedPrice']}}" name="cardpayamount">
                    @else

                    <input type="hidden" id="getFreeArray" value="{{$dataset['price']}}" name="cardpayamount">
                    @endif
                    <input type="hidden" id="examid" value="{{$dataset['examid']}}" name="examid">
