...
<head>
  ...
  <title>E Tutory.lk | View Audit Report</title>
              ...
              <br>
                <table class="table table-bordered">
                  ...
                  <tbody>
                    ...
                    @php
                    $auditActionDataGruped =  DB::table('auditentriesreport')->where([                                                                                             
                                        ['created_at', '>=', $dataset['fromDateSql']],       
                                        ['created_at', '<=', $dataset['toDateValSql']],       
                                        ])->groupBy('action')->get();
                    @endphp 

                    @foreach( $auditActionDataGruped as $auditAction )

                        
                        @php
                        $auditActionOnDataGruped =  DB::table('auditentriesreport')->where([                                                                                             
                                            ['created_at', '>=', $dataset['fromDateSql']],       
                                            ['created_at', '<=', $dataset['toDateValSql']],       
                                            ['action', '=',$auditAction->action], 
                                            ])->groupBy('actionon')->get();
                        @endphp 

                        @foreach($auditActionOnDataGruped as $actionOn)
                ...
                <form role="form" action="{{url('/auditreportPDF') }}" method="post" enctype="multipart/form-data">
                {{csrf_field()}}
