...
<head>
  ...
  <title>E Tutory.lk | Pricing</title>
        ...
        <div class="container-fluid">
          ...
          <div class="row">
            <div class="col-md-8">
              <div class="card">
                <div class="card-body">
                  <a href="{{url('/addnewpricing')}}" class="col-sm-3 btn btn-block btn-primary">Add New Pricing</a>
                </div>
              </div>
            </div>
          </div>
                  ...
                  <table class="table table-bordered">
                    <thead>
                      <tr>
                        ...
                        <th>Grade</th>
                        <th>Subject</th>
                        <th class="text-right">Price Per Papaer</th>
                        <th style="width: 40px">Action</th>
                      </tr>
                    </thead>
                    <tbody>
                      @if(count($dataset) > 0 )
                      @foreach($dataset['pricingdata']->all() as $pricing)
                        <tr>
                          <td>{{$pricing->id}}</td>
                          <td>{{$pricing->examtypename}}</td>
                          <td>{{$pricing->gradename}}</td>
                          <td>{{$pricing->subjectname}}</td>
                          <td class="text-right">{{number_format($pricing->price,2)}}</td>
                          <td>
                          <a href="{{url('/managepricing/'.$pricing->id)}}" class="btn btn-block btn-outline-info btn-xs">Edit</a>
                          </td>
                        </tr>
                      @endforeach
                      @endif
